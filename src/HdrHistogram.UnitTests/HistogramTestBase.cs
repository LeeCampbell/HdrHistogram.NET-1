...
namespace HdrHistogram.UnitTests
{
    public abstract class HistogramTestBase
    {
        ...
        public void RecordValueWithCount_increments_TotalCount(long multiplier)
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                longHistogram.RecordValueWithCount(i, multiplier);
                Assert.AreEqual(i * multiplier, longHistogram.TotalCount);
            }
        }
        ...
        public void RecordValueWithCount_increments_CountAtValue(long multiplier)
        {
            ...
            for (int i = 1; i < 5; i++)
            {
                longHistogram.RecordValueWithCount(TestValueLevel, multiplier);
                Assert.AreEqual(i * multiplier, longHistogram.GetCountAtValue(TestValueLevel));
            }
        }
        ...
        {
            var longHistogram = Create(HighestTrackableValue, NumberOfSignificantValueDigits);
            Assert.Throws<IndexOutOfRangeException>(() => longHistogram.RecordValueWithCount(HighestTrackableValue * 3, 10));
        }
