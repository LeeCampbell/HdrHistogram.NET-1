...
namespace HdrHistogram
{
    ...
    /// </summary>
    /// <remarks>
    /// A <see cref="ConcurrentHistogram"/> guarantees lossless recording of values into the histogram even when the histogram is updated by multiple threads.
    /// <p>
    /// It is important to note that concurrent recording is the only thread-safe behavior provided by <seealso cref="ConcurrentHistogram"/>.
    ...
    public class ConcurrentHistogram : HistogramBase
    {
        ...
        private readonly AtomicLongArray _activeCounts;
        private long _totalCount = 0L;


        /// <summary>
        /// Construct a <see cref="ConcurrentHistogram"/> given the lowest and highest values to be tracked and a number of significant decimal digits.
        ...
        public ConcurrentHistogram(long lowestTrackableValue, long highestTrackableValue, int numberOfSignificantValueDigits) 
        ...
        /// </remarks>
        public ConcurrentHistogram(long instanceId, long lowestTrackableValue, long highestTrackableValue, int numberOfSignificantValueDigits) 
        ...
        {
            var copy = new ConcurrentHistogram(InstanceId, LowestTrackableValue, HighestTrackableValue, NumberOfSignificantValueDigits);
            copy.Add(this);
            return copy;
        }
        ...
        protected override long GetCountAtIndex(int index)
        {
            try
            {
                ...
                Debug.Assert(base.CountsArrayLength == _activeCounts.Length);

                return _activeCounts[index];
            }
            finally
            {
                _wrp.ReaderUnlock();
        ...
        protected override void IncrementCountAtIndex(int index)
        {
            ...
            try
            {
                _activeCounts.IncrementAndGet(index);
        ...
        protected override void AddToCountAtIndex(int index, long addend)
        {
            ...
            try
            {
                _activeCounts.AddAndGet(index, addend);
        ...
        protected override void ClearCounts()
        {
            try
            {
                _wrp.ReaderLock();
                Debug.Assert(CountsArrayLength == _activeCounts.Length);
                for (int i = 0; i < _activeCounts.Length; i++)
                {
                    _activeCounts[i] = 0;
                }
                TotalCount = 0;
